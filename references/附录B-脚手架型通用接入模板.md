...
> **本章角色**：[全局地图 附录 B](00-全局地图.md) 的展开正文。为脚手架型项目提供**可复用的通用模板**，帮助快速满足 7.5.1 红线清单、2.12.5 护栏接入位置、2.12.7 评估挂钩接入位置、2.7 反思回路接入位置；B.5 是 7.5.2-7.5.6 的**文档级可选升级**，不要求仓库已有完整 `tests/security/` 树。
...
> **使用方式**：检查者发现脚手架型项目在 2.7/2.12.5/2.12.7/7.5.1 等检查点判"警示"或"不达标"时，**必须**在改进清单中引用本附录对应模板（B.1-B.5）作为补件路径，不得只写"建议补文档"；被检方按模板补充后复评，即可从"警示/不达标"升级到"达标（接入位置级）"（7.5.1 套 B.1 升"达标"）。G1 + 脚手架下 7.5.2-7.5.6 缺目录只记警示，**不因未套 B.5 记不达标**。
>
...
**适用范围**：脚手架型项目（7.5.2-7.5.6 按 G1 + 脚手架型口径为警示）。B.5 是**文档级可选升级**：在 README 或 `docs/` 写清扩展位置即可，**不要求仓库已有完整 `tests/security/` 树**。缺该树不得记不达标。
...
**脚手架型项目使用说明**：
- G1 + 脚手架下 7.5.2-7.5.6 **缺目录 = 警示，不是不达标**。套 B.5 写清扩展位置（不必已建满目录）可升"达标（接入位置级）"；不套也可保持警示。
...
| 5 | 7.5.2-7.5.6 安全测试 | 警示 | 可选套用 B.5，在文档中写清扩展位置（不必已建满 `tests/security/` 树） | 仍可保持警示；套文档后可达标（接入位置级）。**不得因缺目录记不达标** |
